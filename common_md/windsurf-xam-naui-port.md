...
1. **Set scope to "Entire Solution"**
1. **Replace these patterns:**

**Find:** `xmlns="http://xamarin.com/schemas/2014/forms"` **Replace:** `xmlns="http://schemas.microsoft.com/dotnet/2021/maui"`

**Find:** `xmlns:d="http://xamarin.com/schemas/2014/forms/design"` **Replace:** *(delete this line - not needed in MAUI)*

**Find:** `xmlns:mc="http://schemas.openxmlformats.org/markup-compatibility/2006"` **Replace:** *(delete this line - not needed in MAUI)*

**Find:** `mc:Ignorable="d"` **Replace:** *(delete this line - not needed in MAUI)*
...
1. **Use Find & Replace in Visual Studio** to fix all XAML files at once:
   
   * Find: `xmlns="http://xamarin.com/schemas/2014/forms"`
   * Replace: `xmlns="http://schemas.microsoft.com/dotnet/2021/maui"`
1. **Remove old Xamarin references** from XAML files:
...
1. **Try building again** after these changes
...
1. ✅ **Remove old using statements** from individual files
1. ✅ **Let GlobalUsings.cs handle it** for the entire project
...
1. **Use Find & Replace** to fix the remaining 29 XAML files with the same namespace change
1. **Remove old using statements** from your C# files (since GlobalUsings.cs will handle them)
...
1. **`SQLitePCLRaw.bundle_green`** - Native SQLite binaries for all platforms
...
1. **Restore packages**: Run `dotnet restore` in the terminal
1. **Try building again**: Run `dotnet build`
...
1. **Step 2:** Compiles your project with the packages available
...
1. Putting `ios:Page.UseSafeArea="true"` on the same line or properly formatted
...
1. **Parser Requirement**: The XAML parser expects `attribute="value"` format
1. **String Delimiter**: Quotes tell the parser where the attribute value starts and ends
...
1. **Broken line breaks** in XML attributes
1. **Other malformed XML syntax**
...
1. ✅ The GlobalUsings.cs fix I suggested
...
1. **Keep your current GlobalUsings.cs** as it is
1. **Run `dotnet build`** to test
...
1. ✅ **Used alias `AppPrefs`** to avoid naming conflict with your local Preferences() method
...
1. ✅ **Used `Microsoft.Maui.Storage.Preferences.Default.Get()`** - the correct MAUI syntax
1. ✅ **Avoided naming conflict** with your local Preferences() method
...
1. Set the target to **Android**
1. Select your Android emulator or connected device
...
1. Extract to a folder like: `C:\Android\cmdline-tools`
...
1. Extract to: `C:\adb-tools`
1. Add `C:\adb-tools` to your PATH
...
1. Tap **Build Number** 7 times (enables Developer Options)
...
1. Use the built-in MAUI debugging
1. Select Android target and your device
...
1. **Open the app drawer** (swipe up or tap apps icon)
1. **Look for "NICVC" app**
1. **Tap to launch it**
...
1. **Look for "NICVC" in the list**
1. **Or check the app drawer for a new app**
...
1. **Open your NICVC.sln or NICVC.csproj**
1. **Set target to Android**
1. **Select your device (R9ZY40RDV0K)**
...
1. **Open 2 terminals**
1. **Terminal 1**: Run the logcat monitoring command
1. **Terminal 2**: Launch the app
1. **Watch Terminal 1** for crash details when app crashes
...
adb -s R9ZY40RDV0K logcat \*:E > error_logs\_%date:~-4,4%%date:~-10,2%%date:~-7,2%\_%time:~0,2%%time:~3,2%.txt
...
1. **The `DatabaseHelper` class itself has an issue**
...
1. **Which exact line is failing**
1. **What the actual error message is**
...
1. **Which exact step is failing** (assembly, stream, JSON parsing, etc.)
1. **What the actual error message is**
...
1. ✅ **Embedded resource configuration** (languagejson.txt)
1. ✅ **Missing XAML style** (NavigationLabelColor)
...
1. **Find line 14** with `DependencyService.Get<ISQLite>().GetConnection()`
1. **Replace** it with DatabaseHelper.GetConnection("CicVc.db3")
1. **Save** the file
